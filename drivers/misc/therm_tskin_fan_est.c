...
#include <linux/therm_tskin_est.h>
...
static void fan_set_trip_temp_hyst(struct therm_tskin_fan_est *est, int trip,
...
{
	int i, group, trip_index = 0;
	s64 dev_temp[MAX_SUBDEVICE_GROUP] = {0, };
	s32 scale_coeff, offset_coeff;
	s64 dev_temp_max = 0;
	int temp = 0;
	int fprofile;
	...
	struct therm_tskin_fan_est *est = container_of(
					dwork,
	...
	for (i = 0; i < est->ndevs; i++) {
		if (est->devs[i].get_temp(est->devs[i].dev_data, &temp))
			continue;
		est->devs[i].temp = temp;
	}

	for (i = 0; i < est->ndevs; i++) {
		fprofile = est->current_profile;
		if (fprofile >= MODES_LEN)
			fprofile = MODES_LEN - 1;
		group = est->devs[i].group;

		scale_coeff = est->devs[i].coeffs_mode[fprofile][0];
		offset_coeff = est->devs[i].coeffs_mode[fprofile][1];
		dev_temp[group] = (est->devs[i].temp * scale_coeff) +
				  (offset_coeff * 1000);
		dev_temp[group] = (dev_temp[group] / 10000) + 500;
	}

	for (i = 0; i < MAX_SUBDEVICE_GROUP; i++)
		dev_temp_max = max(dev_temp_max, dev_temp[i]);

	est->cur_temp = dev_temp_max + est->toffset;
...
{
	int i;
	struct therm_tskin_fan_est *est = thz->devdata;
...
static ssize_t show_coeff(struct device *dev,
				...
				char *buf)
{
	struct therm_tskin_fan_est *est = dev_get_drvdata(dev);
	ssize_t len, total_len = 0;
	int i, k, j;

	for (i = 0; i < est->ndevs; i++) {
		for (k = 0; k < MODES_LEN; k++) {
			len = snprintf(buf + total_len, PAGE_SIZE, "[%d] [%d]", i, k);
			total_len += len;
			for (j = 0; j < COEFF_LEN; j++) {
				len = snprintf(buf + total_len, PAGE_SIZE, " %d",
						est->devs[i].coeffs_mode[k][j]);
				total_len += len;
			}
...
{
	struct therm_tskin_fan_est *est = dev_get_drvdata(dev);
	int devid, mode, scount;
	s32 coeff[COEFF_LEN];

	scount = sscanf(buf, "[%d] [%d] %d %d",
			&devid,	&mode, &coeff[0], &coeff[1]);

	if (scount != COEFF_LEN + 2)
		return -1;

	if (devid < 0 || devid >= est->ndevs || mode >= MODES_LEN)
		return -EINVAL;

	/* This has obvious locking issues but don't worry about it */
	memcpy(est->devs[devid].coeffs_mode[mode], coeff,
	       sizeof(coeff[0]) * COEFF_LEN);
				...
				char *buf)
{
	...
	for (i = 0; i < est->ndevs; i++) {
		...
		total_len += snprintf(buf + total_len, PAGE_SIZE, " %d",
				      est->devs[i].temp);
...
static int therm_fan_est_probe(struct platform_device *pdev)
{
	...
	int err = 0;
	int of_err = 0;
	struct therm_tskin_fan_est *est_data;
	...
	pr_debug("THERMAL TSKIN EST start of therm_fan_est_probe.\n");
	if (!pdev)
		return -EINVAL;

	node = pdev->dev.of_node;
	if (!node) {
		pr_err("THERMAL TSKIN EST: dev of_node NULL\n");
		return -EINVAL;
	}

	data_node = of_parse_phandle(node, "shared_data", 0);
	if (!data_node) {
		pr_err("THERMAL TSKIN EST shared data node parsing failed\n");
		return -EINVAL;
	}
	...
	if (of_err) {
		pr_err("THERMAL TSKIN EST: missing ndevs\n");
		return -ENXIO;
	}
	if (child_count != (int)value) {
		pr_err("THERMAL TSKIN EST: ndevs count mismatch\n");
		return -EINVAL;
	}

	est_data = devm_kzalloc(&pdev->dev,
				sizeof(struct therm_tskin_fan_est), GFP_KERNEL);
	if (IS_ERR_OR_NULL(est_data))
		return -ENOMEM;

	est_data->ndevs = child_count;
	pr_info("THERMAL TSKIN EST: found %d subdevs\n", est_data->ndevs);

	of_err |= of_property_read_string(node, "name", &est_data->name);
	if (of_err) {
		pr_err("THERMAL TSKIN EST: name is missing\n");
		err = -ENXIO;
		goto free_est;
	}
	pr_debug("THERMAL TSKIN EST name: %s.\n", est_data->name);

	of_err |= of_property_read_u32(node, "num_resources", &value);
	if (of_err) {
		pr_err("THERMAL TSKIN EST: num_resources is missing\n");
		err = -ENXIO;
		goto free_est;
	}
	est_data->num_resources = value;
	pr_info("THERMAL TSKIN EST num_resources: %d\n", est_data->num_resources);

	of_err |= of_property_read_u32(node, "trip_length", &value);
	if (of_err) {
		pr_err("THERMAL TSKIN EST: missing trip length\n");
		err = -ENXIO;
		goto free_est;
	}
	...
	for_each_child_of_node(data_node, child) {
		pr_info("[THERMAL TSKIN EST subdev %d]\n", j);
		of_err |= of_property_read_string(child, "dev_data",
						&subdevs[j].dev_data);
		if (of_err) {
			pr_err("THERMAL TSKIN EST subdev[%d] dev_data missed\n", j);
			err = -ENXIO;
			goto free_subdevs;
		}
		pr_debug("THERMAL TSKIN EST subdev name: %s\n",
		...
		if (of_property_read_u32(child, "group", &value)) {
			...
			if (value >= MAX_SUBDEVICE_GROUP) {
				pr_err("THERMAL TSKIN EST: group limit exceed\n");
				err = -ENXIO;
				goto free_subdevs;
			} else
		...
		of_err |= of_property_read_u32_array(child, "coeffs_mode0",
			subdevs[j].coeffs_mode[0], COEFF_LEN);
		for (i = 0; i < COEFF_LEN; i++)
			pr_debug("THERMAL TSKIN EST index %d coeffs_mode0 %d\n",
				i, subdevs[j].coeffs_mode[0][i]);
		of_err |= of_property_read_u32_array(child, "coeffs_mode1",
			subdevs[j].coeffs_mode[1], COEFF_LEN);
		for (i = 0; i < COEFF_LEN; i++)
			pr_debug("THERMAL TSKIN EST index %d coeffs_mode1 %d\n",
				i, subdevs[j].coeffs_mode[1][i]);
		j++;
	}
	...
	if (of_err) {
		pr_err("THERMAL TSKIN EST: missing toffset\n");
		err = -ENXIO;
		goto free_subdevs;
	}
	...
	if (of_err) {
		pr_err("THERMAL TSKIN EST: missing polling_period\n");
		err = -ENXIO;
		goto free_subdevs;
	}
	...
	if (est_data->num_profiles > 0) {
		...
		if (of_err) {
			pr_err("THERMAL TSKIN EST: missing default fan profile\n");
			err = -ENXIO;
			goto free_subdevs;
		}
		pr_info("THERMAL TSKIN EST: Found %d profiles, default profile is %s\n",
		...
		for_each_available_child_of_node (base_profile_node, profile_node) {
			...
			if (of_err) {
				pr_err("THERMAL TSKIN EST: active trip temps failed to parse.\n");
				err = -ENXIO;
				goto free_subdevs;
			}
			...
			of_err |= of_property_read_u32_array(profile_node,
					...
					(size_t) est_data->trip_length);
			if (of_err) {
				pr_err("THERMAL TSKIN EST: active hysteresis failed to parse.\n");
				err = -ENXIO;
				goto free_subdevs;
			}
			i++;
	...
	for (i = 0; i < est_data->trip_length; i++)
		pr_debug("THERMAL TSKIN EST index %d: trip_temp %d, hyst %d\n",
	...
	for (i = 1; i < MAX_ACTIVE_STATES; i++) {
		...
		if (((i + 1) < MAX_ACTIVE_STATES)
			...
			pr_err("THERMAL TSKIN EST: active hysteresis invalid\n");
			err = -EINVAL;
			goto free_subdevs;
		}
	}
	for (i = 0; i < MAX_ACTIVE_STATES; i++)
		pr_debug("THERMAL TSKIN EST index %d: trip_temps_hyst %d\n",
	...
	for (i = 0; i < est_data->ndevs; i++) {
		...
		dev->temp = temp;
		pr_debug("THERMAL TSKIN EST init dev[%d] temp to %d\n", i, temp);
	}

	of_err |= of_property_read_string(data_node, "cdev_type",
						&est_data->cdev_type);
	if (of_err) {
		pr_err("THERMAL TSKIN EST: cdev_type is missing\n");
		err = -EINVAL;
		goto free_subdevs;
	}
	pr_debug("THERMAL TSKIN EST cdev_type: %s.\n", est_data->cdev_type);
	...
	of_err |= of_property_read_string(data_node, "tzp_governor_name",
						&gov_name);
	if (of_err) {
		pr_err("THERMAL TSKIN EST: governor name is missing\n");
		err = -EINVAL;
		goto free_tzp;
	}
	strcpy(tzp->governor_name, gov_name);
	pr_debug("THERMAL TSKIN EST governor name: %s\n", tzp->governor_name);
	...
	if (IS_ERR_OR_NULL(est_data->thz)) {
		pr_err("THERMAL TSKIN EST: thz register failed\n");
		err = -EINVAL;
		goto free_tzp;
	}
	pr_info("THERMAL TSKIN EST: thz register success.\n");
	...
	pr_info("THERMAL TSKIN EST: end of probe, return err: %d\n", err);
	return err;
...
{
	struct therm_tskin_fan_est *est = platform_get_drvdata(pdev);
	int i;
							...
							pm_message_t state)
{
	...
	pr_debug("therm-tskin-fan-est: %s, cur_temp:%ld", __func__, est->cur_temp);
	cancel_delayed_work(&est->therm_fan_est_work);
	est->current_trip_level = 0;
...
{
	struct therm_tskin_fan_est *est = platform_get_drvdata(pdev);
	pr_info("therm-tskin-fan-est: shutting down\n");
...
static struct platform_driver therm_tskin_fan_est_driver = {
	.driver = {
		.name  = "therm-tskin-fan-est-driver",
		.owner = THIS_MODULE,
...
module_platform_driver(therm_tskin_fan_est_driver);

MODULE_DESCRIPTION("fan tskin thermal estimator");
